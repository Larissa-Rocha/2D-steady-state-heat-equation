...
error = 1;        %inicializando o erro
k = 0;            %inicializando a iteração
omega = 1.75;        %relaxation parameter
...
T = ones(nx,ny);            %inicializando o vetor temperatura                 
...
while error > tol
    for i = 2:nx-1
        T(1,i) = omega*(0.25*(T(1,i+1) + T(1,i-1) + 2*T(2,i))) + (1-omega)*Told(1,i); %T na base
    end %fim do for do x
    
    for j = 2:ny-1
        for i = 2:nx-1
            T(j,i) = omega*(0.25*(T(j,i+1) + T(j,i-1) + T(j+1,i) + T(j-1,i))) + (1-omega)*Told(j,i);
        end %fim do for do y
    end %fim do for do x
...
%     xlabel('coordenada x')
%     ylabel('coordenada y')
%     title('Distribuição de temperatura (omega = 1.9)');
